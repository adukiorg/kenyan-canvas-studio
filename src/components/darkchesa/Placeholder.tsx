...
export const Placeholder = ({ label, caption, className, ratio = "portrait", image, alt }: PlaceholderProps) => {
  return (
    <div
      className={cn(
        "group relative w-full overflow-hidden border border-border/60",
        !image && "placeholder-art",
        image && "bg-charcoal-soft",
        ratioMap[ratio],
        className,
      )}
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-1/3 bg-gradient-to-t from-charcoal/80 to-transparent opacity-80" />
    </div>
  );
};
